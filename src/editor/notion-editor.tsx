...
import { getCursorOffset, setCursorPosition } from "./cursor";
import {
...
export const NotionEditor = ({ content, handleChange }: NotionEditorProps) => {
  ...
  const updateBlockState = (blockData: EditableBlockContent) => {
    ...
    const updateIdx = newState.findIndex((block) => block.id === blockData.id);
  ...
  const handleMouseUp = () => {
    ...
    if (top < 200) {
      top = selectionRect.top + distanceFromTop + selectionRect.height + 10;
